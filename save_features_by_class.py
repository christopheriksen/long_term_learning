...
def main():
    ...
    features_filename_base = '/home/scatha/lifelong_object_learning/long_term_learning/pytorch_rgbd_features_'
    labels_filename_base = '/home/scatha/lifelong_object_learning/long_term_learning/pytorch_rgbd_labels_'
    ############################################
    ...
    num_features_by_class = [0 for i in range(num_classes)]
    ...
    for i, (input, target) in enumerate(data_loader):
        ...
        features_by_class[target[0]].append(features[0])
        num_features_by_class[target[0]] += 1


    num_subsets = int(math.ceil(float(num_classes)/10))

    # group classes into subsets and write each example per subset
    for subset_index in list(range(num_subsets))[:-1]:

        features_file = open(features_filename_base + str(subset_index) + '.txt','w')
        labels_file = open(labels_filename_base + str(subset_index) + '.txt','w')

        num_data_points = sum(num_features_by_class[subset_index*10: subset_index*10 + 10])
        features_file.write(str(num_data_points) + ' ' + str(num_features) + '\n')

        for class_index in list(range(num_classes))[subset_index*10: subset_index*10 + 10]:

            for example in features_by_class[class_index]:
                features_file.write(str(example[0]))
                for feature in example[1:]:
                    features_file.write(' ' + str(feature))
                features_file.write('\n')
                labels_file.write(str(class_index) + '\n')
    ...
    # last subset
    subset_index = list(range(num_subsets))[-1]
    features_file = open(features_filename_base + str(subset_index) + '.txt','w')
    labels_file = open(labels_filename_base + str(subset_index) + '.txt','w')

    num_data_points = sum(num_features_by_class[subset_index*10:])
    features_file.write(str(num_data_points) + ' ' + str(num_features) + '\n')

    for class_index in list(range(num_classes))[subset_index*10:]:
