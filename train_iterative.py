...
def train_distillation(train_dataset, coreset, model, criterion, optimizer, batch_size, workers, num_classes):
    ...
    print (torch.utils.data.dataset.BatchSampler(torch.utils.data.dataset.RandomSampler(range(total_num)), batch_size=batch_size, drop_last=False))

    print(torch.utils.data.dataset.SubsetRandomSampler(list(range(total_num)), batch_size=batch_size, drop_last=False))
